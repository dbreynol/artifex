...
7. (Optional, but mandatory if you finish 1-6 in class) <code>q6()</code>: function that finds the sum of squared errors for $(\beta_0, \beta_1)$ in a simple linear regression model. Use output from this function and a 3d plot using <code>persp()</code> to obtain least squares estimates. Verify using <code>lm()</code>.
